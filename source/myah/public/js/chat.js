...
{
    const html = selector.html().replace(/!!\[link href="(.*?)" value="(.*?)"\]/g, '<a href="$1">$2</a>');
    selector.html(html);
}
...
/*
case "file/prog":
    //server side: {"action": u"file/prog", "user": username, "name": filename, "size": filesize, "id": namehash,
"progress": 0}
    //Get bubble if exists, create a new one if not
    var $bubble = $('div#fp-' + e.id).length ? $('div#fp-' + e.id) : $('<div class="bubble notify" id="fp-' + e.id +
'">' + e.user + ' - uploading: ' + LChat.shortenFileName(e.name, 50) + '(' + e.size + 'B)\
                                                                            <div class="prgBar" id="pb-' + e.id + '">\
                                                                            <div class="prgBarBack" id="pbb-' + e.id +
'">&nbsp;</div>\
                                                                            <div class="prgBarFront" id="pbf-' + e.id +
'">&nbsp;</div>\
                                                                            <div class="prgBarText" id="pbt-' + e.id +
'">' + e.progress + '%\
                                                                        </div>\
...
case "file/done":
    //server side: {"action": u"file/done", "user": username, "name": filename, "id": staticMsgDict[namehash], "url":
fileurl}
    $('div#fp-' + e.id).empty();
    $('div#fp-' + e.id).html(e.user + ' - uploaded: ' + LChat.shortenFileName(e.name, 50) + '<br/><a href="' + e.url +
'" download="' + e.name + '">[DOWNLOAD]</a>'); LChat.addFile(e.key, e.name, e.url); break;
...
function displayMessage(postid, username, message, timestamp)
{
    ...
    const bubble_style = `bubble bubble-spk${(other ? ' other' : '')}`;
    const bubble_user_style = `bubble-user${(other ? ' other' : '')}`;
    const template = '<div id="{{postid}}" class="{{bubble_style}}"><div class="bubble-content">{{message}}</div><div
class="{{bubble_user_style}}"><strong>{{username}}</strong><br/><small>{{date}}</small></div></div>'; const html =
Mustache.render(template, { postid : postid, bubble_style : bubble_style, bubble_user_style : bubble_user_style, message
: message, username : username, date : timestampToDateString(timestamp)
    });
    $("#messages").prepend(html);
    $(`#messages>div#${postid}`).linkify();
    parseImages($(`#messages>div#${postid}>div>a`));
